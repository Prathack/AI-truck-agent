...
const ActivityLog = ({ logs }) => {
  ...
  return (
  <div className="glass-panel rounded-lg sm:rounded-xl lg:rounded-2xl xl:rounded-3xl border border-white/5 overflow-hidden flex flex-col h-95 sm:h-115 md:h-130 lg:h-150">
  {/* Header */}
  <div className="px-3 py-2 sm:px-4 sm:py-3 lg:px-5 border-b border-white/5 bg-white/5 flex items-center justify-between shrink-0">
    <div className="flex items-center gap-2 sm:gap-2.5">
      <Clock className="w-3 h-3 sm:w-3.5 sm:h-3.5 md:w-4 md:h-4 text-white/30 shrink-0" />
    ...
    <AnimatePresence initial={false}>
      {logs.length === 0 ? (
        ...
        >
          ...
          <p className="text-[10px] sm:text-xs md:text-sm text-white/30 max-w-60 sm:max-w-xs">
        ...
        logs.map((log) => (
          ...
          >
            {/* Timestamp */}
            <span className="hidden sm:block text-[8px] sm:text-[9px] md:text-[10px] font-mono text-white/20 whitespace-nowrap mt-0.5 shrink-0">
              [{log.timestamp}]
            </span>
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[9px] sm:text-[10px] md:text-xs text-white/50 inline wrap-break-word">
                {log.message}
              </p>
            </div>
          </motion.div>
